...
test("If a course is added to the course list it should be in the course list", async () => {
    ...
    const course: Course = await courseService.createCourse("ABC123", "Sjö","test", 50);
...
test("The getCourse method should return the spcified course", async () => {
    ...
    const course1: Course = await courseService.createCourse("ABC123", "Sjö","test", 50);
    const course2: Course = await courseService.createCourse("DEF456", "Sjö","test2", 60);
...
test("The checkAnswer method should return true if the first course has a higher failrate than the second course", async () => {
    const courseService = new CourseService();
    const course1: Course = await courseService.createCourse("ABC123", "Sjö","test", 60);
    const course2: Course = await courseService.createCourse("DEF456", "Sjö","test2", 50);
